#!/usr/bin/env python3
"""Build the cocopilot-evolve journey website from markdown sources.

Reads JOURNAL.md, IDENTITY.md and DAY_COUNT from the repo root, then
writes docs/index.html and docs/style.css.  Designed to run automatically
after every evolution session so the public site stays up to date.
...
REPO_URL = "https://github.com/audiohacking/cocopilot-evolve"
SITE_TITLE = "cocopilot"
TAGLINE = "a self-evolving agent growing up in public"
...
def parse_journal(content: str) -> list:
    entries = []
...
def parse_identity(content: str) -> dict:
    intro_lines: list = []
    rules: list = []
...
<head>
  ...
  <title>{title} \u2014 Day {day_count}</title>
  <meta name="description" \
content="A self-evolving coding agent powered by GitHub Copilot. Currently on Day {day_count}.">
...
def build() -> None:
    ...
    day_count_raw = read_file("DAY_COUNT").strip()
    day_count = day_count_raw if day_count_raw.isdigit() else "0"

    journal_content = read_file("JOURNAL.md")
    identity_content = read_file("IDENTITY.md")

    entries = parse_journal(journal_content)
    identity = parse_identity(identity_content)

    journal_html = render_journal(entries)
    identity_html = render_identity(identity)
    ...
    print(f"Site built → docs/ (Day {day_count}, {len(entries)} journal entries)")
